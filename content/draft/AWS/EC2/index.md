...
他のインスタンスに付け替えることができる。ただし**AZをまたぐことはできない**。別Aで利用したいときは、スナップショットを取得、別AZの方でスナップショットか新しいボリュームを作成するようにする。

基本的に複数のインスタンスを接続させることはできない。プロビジョンドIOPSのみできる。

## EBSスナップショット

スナップショットをとり、S3に保存することでバックアップを行う。2回目以降は増分バックアップ。S3に保存するのでAZをまたいで共有可能。
...
スナップショットは自動的に暗号化される。全てのボリュームタイプで有効だが、インスタンスタイプには制限がある（ただし、現行世代は全て対応している）。
...
1つのインスタンスに複数のENIをアタッチ可能。別々のSSL証明書を使用することが可能。

## 拡張ネットワーキング

高性能ネットワーキング機能の提供。追加料金なし。**HVM仮想化環境**のVPCでのみ利用可能。
