...
デフォルトでは**空行は除いて**番号を降る。

`-b`オプションで番号振りに関して設定できる。

`a`で全ての行、`-t`で空行以外、`-n`で空行に番号を振る。
...
`xz`コマンドで、xz形式でファイルを圧縮できます。

`xz -d`か`unxz`コマンドで展開できる。
...
ファイルの種類を表示。ファイルの最終アクセス時刻が更新される。


## stat

ファイルのファイルやファイルシステムのinode番号やタイムスタンプ（アクセス時刻や修正時刻）などの詳細情報を表示できます。

## touch

`-t`でアクセス時刻、修正時刻を修正。`-m`で修正時刻。

## ls

-Fでファイルタイプ。`-l`で詳細情報。`-R`で再帰的に表示
