...
次に、`text.txt`の内容を更新してみましょう。`echo -n "I love Git." > text.txt`とコマンドラインで実行します（追記ではなく上書きを行います）。そして`git add text.txt`を実行します。blobオブジェクトが出来ているはずなので確認してみます。
...
このように、`.git/objects/`配下にblobオブジェクトは3つありますが、`index`には2つのファイルしか記録されていません。よく考えれば、最初に`git add`した時の`e69de2~`も残っていますね（今はどこからも参照されていないにも関わらず）。

ここにHex Editorの図を挿入

どうやら、不要になったblobオブジェクトは`index`からは消えるものの、`.git/objects`には残されるようです。
...
danglingオブジェクトがずっと残り続けるかというとそうではなく、前述したとおり`git gc`（Gitのガベージコレクション）によって削除されるようです。`git gc`はGitによって何らかの契機をもとに自動的に実行されますが、手動で実行することも可能です。

さすがにGCについては本題から逸れるので今回は割愛しますが、もしかしたらこの後の記事で登場するかもしれません（調べてみるとややこしそうなので、実現するかは微妙）。
