...
従来のWebサイトの例として、WebサーバーとDBサーバーを組み合わせた構成を考えます。ブログコンテンツ（記事のタイトルや本文、画像など）はDBサーバーに保存、クライアントからのアクセスがあった時にWebサーバーがPHPなどがテンプレートを元にHTMLファイルを作成、クライアントに返送するというパターンです（いわゆる**LAMP構成**というやつですね）。
