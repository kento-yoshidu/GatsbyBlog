...
私が思うにJamstackとは「APIなどを通じ静的ファイルを事前生成し、CDNなどのホスティングサービスを使って配信するアーキテクチャー」だと考えています。これでも長くて何のこっちゃと思うかもしれませんが、例えば、静的サイトジェネレーター + HeadlessCMS + CDNを組み合わせたアーキテクチャーを想像してもらえれば、幾分分かりやすいんじゃないかと思います。
...
従来のWebサイトの例として、WebサーバーとDBサーバーを組み合わせた構成を考えます。ブログコンテンツ（記事のタイトルや本文、画像など）はDBサーバーに保存、クライアントからのアクセスがあった時にWebサーバーがPHPなどでHTMLファイルを作成、クライアントに返送するというパターンです（いわゆる**LAMP構成**というやつですね）。
...
まず、一つ目が見出しにもある静的ファイルの**事前**生成です。

Jamstackでは**コンテンツの更新があった時**に、静的サイトジェネレーターを利用して、コンテンツデータを取得しHTMLやCSSなどの静的ファイルを生成、生成された静的ファイルをCDNの配信サーバーにコピーします（CDNに関しては後ほど説明します）。

![](images/image02.png)

いわば、ユーザーへWebサイトを公開する前の準備段階です。

そして、CDNのサーバーへデータコピーが完了したら、ユーザーからのアクセスが可能になります。ここでは①HTTPリクエストを投げ、②静的ファイルを返す、という単純な流れです。
...
JamstackではCDNというサービスを使用して、静的サイトジェネレーターで出力した静的なファイルを配信します。
