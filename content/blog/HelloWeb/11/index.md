...
もっと子孫要素を増やしてネストさせてみましょう。以下のHTMLは、`p`クラスの中に`span`要素を用意しさらにその中に`span`要素を用意した例です。
...
上記の例では`span`要素に同じ`border`プロパティを用意しましたが、`inherit`という値を使用することで、**本来は継承されないプロパティを継承する**ことができます。
