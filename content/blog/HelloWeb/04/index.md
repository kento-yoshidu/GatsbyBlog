...
これまでHTMLに関してざっとハンズオンを行ってきましたが、ちゃんとした文法を説明してませんでした。ここでHTMLの基本的な書き方や記述ルールを覚えましょう。

これまでは`index.html`を使っていましたが、今回は文法の確認なので使いません。適当なHTMLファイルを作成して、そちらを使ってください。例えば`sample.html`などを作成し、以下のように記述してください。`index.html`をコピーしてもいいですよ。
...
`2`と`3`の間に空白行がありますね。ｍ、また、`3`の左側にはタブでスペースが空いています。しかし、これらは実際の表示には反映されていないことが分かります。
