...
```typescript{2}:title=src/components/search.tsx
import * as React from "react"
...
UIの作成にはuseStateとuseEffectを使用します。入力された文字列を保持するフックと、条件によって絞り込まれた記事すべてを保持するフックを用意します。
...
今回はインクリメンタルサーチですから、入力ボックスに1文字入力されるたびにJSONファイルを走査して結果を書き換える必要があります。

まずはinput要素に`onChange`属性を定義し、入力された文字列を`setInputedKeyword`に渡すようにします。
...
続けて、`useEffect`を定義し、第二引数に`inputedKeyword`を渡します。これで入力フォームに書き込まれるたびに`useEffect`が実行される状態になりました。`useEffect`には`console.log(inputedKeyword)`などと記述し、フォームに1文字入力されるたびにコンソール出力されることを確認してください。
...
const Search = () => {
  ...
  useEffect(() => {
    ...
    // ここにinputedKeywordの中身を使ってJSONファイルを走査し、
...
それでは`useEffect`の処理を記述します。まずは入力された文字列を全て小文字に変換し、`slowerKeywords`といった変数に保管します。
